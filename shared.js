...
async function awardTaskPoints(artistSlug,isOnTime){
  if(!artistSlug)return;
  const pts=isOnTime?2:1;
  const{data:lvl}=await db.from('artist_level').select('task_completion_points').eq('artist_slug',artistSlug).maybeSingle();
  const newVal=(lvl?.task_completion_points||0)+pts;
  await db.from('artist_level').update({task_completion_points:newVal}).eq('artist_slug',artistSlug);
  showPointsPop(pts);
}
...
function showPointsPop(points){
  ...
  el.style.cssText=`
    position:fixed;left:50%;bottom:90px;z-index:9999;
