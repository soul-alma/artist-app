...
// inline, without an assignee/due-date/status of their own. Uses a custom
// clickable box (matching the platform-checkbox style used elsewhere)
// instead of a native <input type="checkbox"> — native checkboxes inside
// a contenteditable region are notoriously unreliable to click/toggle
// across browsers, so this sidesteps that entirely.

function clCheckHtml(clid,checked){
  return`<span class="cl-check" contenteditable="false" data-clid="${clid}" data-checked="${checked?'true':'false'}" style="display:inline-flex;align-items:center;justify-content:center;width:20px;height:20px;border-radius:6px;border:2px solid ${checked?'var(--accent)':'var(--border2)'};background:${checked?'var(--accent)':'var(--bg)'};vertical-align:middle;cursor:pointer;flex-shrink:0;margin-right:5px;transition:all .15s">${checked?'<i class="ti ti-check" style="color:white;font-size:12px;pointer-events:none"></i>':''}</span>`;
}

function insertChecklistItem(){
  const clid='cl'+Date.now().toString(36)+Math.random().toString(36).slice(2,6);
  document.execCommand('insertHTML',false,clCheckHtml(clid,false)+'\u200b');
  // Mark this editor as "inside a checklist" so Enter continues the list.
  const sel=window.getSelection();
  const editor=sel?.anchorNode&&(sel.anchorNode.nodeType===1?sel.anchorNode:sel.anchorNode.parentElement)?.closest?.('.copy-editor');
  if(editor)editor._inChecklist=true;
}

// Flips the visual checked state of a custom checklist box in place.
function _flipClVisual(el){
  const checked=el.dataset.checked!=='true';
  el.dataset.checked=checked?'true':'false';
  el.style.border=`2px solid ${checked?'var(--accent)':'var(--border2)'}`;
  el.style.background=checked?'var(--accent)':'var(--bg)';
  el.innerHTML=checked?'<i class="ti ti-check" style="color:white;font-size:12px;pointer-events:none"></i>':'';
  return checked;
}

// Editor context: toggling here is purely visual — the whole task gets
// saved together when the form is submitted, same as any other edit.
document.addEventListener('click',(e)=>{
  const box=e.target.closest?.('.cl-check');
  if(!box)return;
  if(box.dataset.interactive)return; // live (persists) or off (inert) — both handled elsewhere/not here
  if(!box.closest('.copy-editor'))return; // only toggle visually inside an actual editor
  _flipClVisual(box);
});

// Enter continues the checklist automatically; Enter on an empty
// checklist line exits it (standard list-editing convention).
document.addEventListener('keydown',(e)=>{
  if(e.key!=='Enter')return;
  const editor=e.target.closest?.('.copy-editor');
  if(!editor||!editor._inChecklist)return;
  e.preventDefault();
  const sel=window.getSelection();
  if(!sel.rangeCount)return;
  const range=sel.getRangeAt(0);
  const lineText=range.startContainer.textContent?.trim();
  if(!lineText){
    // Empty line — exit the checklist, just insert a normal line break.
    editor._inChecklist=false;
    document.execCommand('insertHTML',false,'<br>');
    return;
  }
  const clid='cl'+Date.now().toString(36)+Math.random().toString(36).slice(2,6);
  document.execCommand('insertHTML',false,'<br>'+clCheckHtml(clid,false)+'\u200b');
});

// Injects (or strips) the live click handler on every checklist box in a
// rendered HTML string. With no taskId (card previews, etc.) boxes are
// left inert — nothing to save a toggle to there, and a toggle that looks
// like it worked but silently doesn't persist would be worse than nothing.
function wireChecklistCheckboxes(html,taskId,field){
  if(!html)return html;
  if(!taskId){
    return html.replace(/class="cl-check"/g,'class="cl-check" data-interactive="off"');
  }
  return html.replace(/<span class="cl-check"/g,`<span class="cl-check" data-interactive="live" onclick="liveToggleChecklistItem(this,'${taskId}','${field}')"`);
}

function updateCheckboxInHtml(html,clid,checked){
  const tmp=document.createElement('div');
  tmp.innerHTML=html;
  const box=tmp.querySelector(`.cl-check[data-clid="${clid}"]`);
  if(box){
    box.dataset.checked=checked?'true':'false';
    box.style.border=`2px solid ${checked?'var(--accent)':'var(--border2)'}`;
    box.style.background=checked?'var(--accent)':'var(--bg)';
    box.innerHTML=checked?'<i class="ti ti-check" style="color:white;font-size:12px;pointer-events:none"></i>':'';
  }
  return tmp.innerHTML;
}

// Read-only/task-detail context: toggles visually AND persists immediately.
async function liveToggleChecklistItem(el,taskId,field){
  const checked=_flipClVisual(el);
  await toggleChecklistItem(taskId,field,el.dataset.clid,checked);
}

// Persists a single checklist toggle straight to the DB.
...
// platforms/platforms_done are jsonb columns — Supabase returns them
// already parsed as real arrays, not JSON strings. Calling JSON.parse()
// on an already-parsed array throws (String([]) is '', so an empty array
// crashes with "Unexpected end of JSON input"). This handles both shapes.
...
function initTTRecDayPicker(){
  ...
  const note=document.getElementById('tt-rec-days-note');
  if(!container)return;
  container.innerHTML='';
  if(note)note.style.display=_ttRecType==='monthly'?'block':'none';
...
function getRecurrenceDates(type,days,from,periods){
  const dates=[];
  // Accept a Date object OR a "YYYY-MM-DD" string. Parsing a date-only
  // string via `new Date(str)` is a classic JS trap — it's interpreted as
  // UTC midnight, not local midnight, silently shifting to the previous
  // day in any timezone behind UTC (this bit us before in the metrics
  // report; same root cause here).
  let d;
  if(from instanceof Date){
    d=new Date(from.getFullYear(),from.getMonth(),from.getDate());
  } else {
    const[fy,fm,fd]=String(from).split('-').map(Number);
    d=new Date(fy,fm-1,fd);
  }
  d.setHours(0,0,0,0);
...
async function getSubtasks(taskId){
  const{data}=await db.from('tasks').select('id,status,due_date').eq('parent_task_id',taskId).is('deleted_at',null);
  return data||[];
}
...
async function awardTaskPoints(artistSlug,isOnTime,taskId){
  if(!artistSlug)return;
  // Never award twice for the same task — otherwise completing, undoing,
  // and re-completing the same task would double (or triple...) its points.
  if(taskId){
    const{data:existing}=await db.from('task_points_log').select('id').eq('task_id',taskId).limit(1);
...
// Registers early, outside any user-triggered flow — registration itself
// doesn't ask for any permission, that happens separately when the
// person opts in to notifications.
if('serviceWorker' in navigator){
  window.addEventListener('load',()=>{
    navigator.serviceWorker.register('/sw.js')
      .then(()=>{if(Notification?.permission==='granted')subscribeToPush();})
      .catch(e=>console.error('[SW] register failed:',e));
  });
}

// Shows the "activate notifications" banner only when it's actually
// relevant: the browser supports it, permission hasn't been granted or
// denied yet, and the person didn't dismiss it recently (avoids nagging
// every single time they open the panel). Checks both possible banner
// IDs — only one exists per app, since each HTML file has its own.
function _notifBannerEl(){
  return document.getElementById('notif-permission-banner')||document.getElementById('mgr-notif-permission-banner');
}

function updateNotifPermissionBanner(){
  const banner=_notifBannerEl();
  if(!banner)return;
  if(!('Notification' in window)){banner.style.display='none';return;}

  if(Notification.permission==='denied'){
    // Can't be requested again from the page once denied — the browser
    // blocks that specifically to stop sites from nagging. Only a manual
    // fix in the browser's own site settings can undo it.
    banner.innerHTML=`<div style="display:flex;align-items:flex-start;gap:10px">
      <i class="ti ti-bell-x" style="font-size:20px;color:var(--danger);flex-shrink:0;margin-top:1px"></i>
      <div style="flex:1">
        <div style="font-size:13px;font-weight:700;color:var(--text);margin-bottom:2px">Las notificaciones están bloqueadas</div>
        <div style="font-size:12px;color:var(--text2)">Se rechazaron antes en este navegador. Para activarlas, entra a la configuración del sitio (icono de candado junto a la URL) y cambia "Notificaciones" a "Permitir", luego recarga la página.</div>
      </div>
    </div>`;
  ...
  if(Notification.permission!=='default'){banner.style.display='none';return;}
  const dismissedAt=localStorage.getItem('notifBannerDismissed');
  if(dismissedAt&&(Date.now()-parseInt(dismissedAt))<7*86400000){banner.style.display='none';return;}
  banner.innerHTML=`<div style="display:flex;align-items:flex-start;gap:10px">
    <i class="ti ti-bell-ringing" style="font-size:20px;color:var(--accent);flex-shrink:0;margin-top:1px"></i>
    <div style="flex:1">
      <div style="font-size:13px;font-weight:700;color:var(--text);margin-bottom:2px">Activa las notificaciones</div>
      <div style="font-size:12px;color:var(--text2);margin-bottom:8px">Para enterarte al instante cuando te mencionen o te asignen algo, incluso con la app cerrada.</div>
      <div style="display:flex;gap:8px">
        <button onclick="requestNotifPermission()" class="btn btn-primary btn-sm">Activar</button>
        <button onclick="dismissNotifBanner()" class="btn btn-ghost btn-sm">Ahora no</button>
      </div>
    </div>
  </div>`;
  banner.style.display='block';
...
function dismissNotifBanner(){
  localStorage.setItem('notifBannerDismissed',String(Date.now()));
  const banner=_notifBannerEl();
  if(banner)banner.style.display='none';
}

// Public VAPID key — safe to embed client-side, that's how it's meant to
// be used (only the PRIVATE key, held only by the server, stays secret).
const VAPID_PUBLIC_KEY='BK2O3MpWQvveDj4OYtXf5ZosAhRsWgGrn3Lk7P5EX3WPa4Gh143BJgDbhy-jU6CTc5XmJTCLeDodDq9Or8-XA8E';
...
// Subscribes THIS device/browser to receive pushes, and saves its unique
// connection info so the server knows where to send them later. Safe to
// call repeatedly — reuses the existing subscription if one already
...
async function subscribeToPush(){
  ...
  try{
    ...
    const recipientToken=window.currentMember?.token
      ||(typeof currentUser!=='undefined'?currentUser:null)
      ||(typeof currentManagerSlug!=='undefined'?currentManagerSlug:null);
    ...
    await db.from('push_subscriptions').upsert([{
      ...
      auth:json.keys.auth,
    }],{onConflict:'endpoint,recipient_token'});
